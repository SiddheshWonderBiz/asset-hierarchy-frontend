    ...
    <section className="w-full">
      <div className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex-1">
          ...
          <p className="text-gray-600 mt-1">Manage your organizational structure</p>
        </div>
        
        <button
          onClick={() => onAddHierarchy(null)}
          className="bg-green-600 hover:to-green-700 text-white py-3 px-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-105 font-medium flex items-center gap-2"
        >
          ...
          <div className="space-y-2">
            {data.children.map((child, index) => (
              <div key={child.id} className="relative">
                {index > 0 && <div className="absolute -top-1 left-0 right-0 h-px bg-gray-200"></div>}
                <NodeItem
                  node={child}
                  onAdd={onAdd}
                  onDelete={onDelete}
